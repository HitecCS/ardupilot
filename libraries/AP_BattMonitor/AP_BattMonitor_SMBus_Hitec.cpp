...
#define BATTMONITOR_HITEC_CELL_COUNT                0x5C    // cell-count register
#define BATTMONITOR_HITEC_CELL_START                0x30    // first cell register
#define BATTMONITOR_HITEC_CURRENT                   0x0a    // current register
#define BATTMONITOR_HITEC_TEMPERATURE               0x08    // temperature register

// TODO: Remove
#define BATTMONITOR_SMBUS_SOLO_BUTTON_DEBOUNCE      6       // button held down for 5 intervals will cause a power off event

// Constructor
AP_BattMonitor_SMBus_Hitec::AP_BattMonitor_SMBus_Hitec(AP_BattMonitor &mon,
...
void AP_BattMonitor_SMBus_Hitec::timer()
{
    uint8_t buff[8];
    ...
    if (_cell_count == 0) {
        if (read_block_bare(BATTMONITOR_HITEC_CELL_COUNT, buff, 2)) {
    ...
    if (_cell_count > 0) {
        ...
        for(uint8_t i = 0; i < _cell_count; ++i) {
            if(read_block_bare(BATTMONITOR_HITEC_CELL_START + i, vbuf, 2)) {
    ...
    // read current
    if (read_block_bare(BATTMONITOR_HITEC_CURRENT, buff, 2) == 2) {
...
bool AP_BattMonitor_SMBus_Hitec::read_temperature() 
{
    uint8_t buff[2];
...
uint8_t AP_BattMonitor_SMBus_Hitec::read_block_bare(uint8_t reg, uint8_t* data, uint8_t max_len) const
{
    // read bytes
